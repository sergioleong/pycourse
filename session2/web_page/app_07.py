...
from chatbot_07 import get_ollama_response
...
def chat():
    ...
    chat_history = []
    chat_id = None
    bot_config = BOTS.get('default', {}) # Default to 'default' bot
    ...
    return render_template('chat_07.html',
                           username=username,
...
def api_chat():
    ...
    current_bot_id = session.get('current_bot_id', 'default') # Get bot_id from session
    db_session = get_db_session()
    current_chat = None

    # Get bot configuration
    bot_config = BOTS.get(current_bot_id, BOTS.get('default', {}))
    ...
    if not current_chat:
        ...
        current_chat = Chat(user_id=user_id, bot_id=current_bot_id, message_count=0, last_message_at=datetime.utcnow())
