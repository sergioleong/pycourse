...
def index():
    ...
    user_chats = db_session.query(Chat).filter_by(user_id=user_id)\
...
def chat():
    ...
    if bot_id_param:
        ...
        new_chat = Chat(user_id=user_id, bot_id=bot_id_param, message_count=0, last_message_at=datetime.utcnow())
    ...
    return render_template('chat_08.html',
                           username=username,
                           chat_history=chat_history,
                           bot_name=bot_name,
                           welcome_message=welcome_message,
                           chat_id=selected_chat_id) # Pass chat_id to the template
